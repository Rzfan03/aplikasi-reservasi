...
layananRouter.get('/', requireAdmin, wrap(async (_req, res) => {
  const items = await prisma.layanan.findMany({ orderBy: { urutan: 'asc' } })
  res.json(items)
}))

layananRouter.post('/', requireAdmin, wrap(async (req, res) => {
...
layananRouter.put('/:id', requireAdmin, wrap(async (req, res) => {
  const { nama, urutan } = req.body
...
layananRouter.delete('/:id', requireAdmin, wrap(async (req, res) => {
...
instansiRouter.get('/', requireAdmin, wrap(async (_req, res) => {
  const items = await prisma.instansi.findMany({ orderBy: { createdAt: 'desc' } })
  res.json(items)
}))

instansiRouter.post('/', requireAdmin, wrap(async (req, res) => {
  const { nama } = req.body
  if (!nama) { res.status(400).json({ error: 'nama is required' }); return }
...
instansiRouter.put('/:id', requireAdmin, wrap(async (req, res) => {
  const { nama } = req.body
  const item = await prisma.instansi.update({ where: { id: req.params.id }, data: { nama } })
  res.json(item)
}))

instansiRouter.delete('/:id', requireAdmin, wrap(async (req, res) => {
